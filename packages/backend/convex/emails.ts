...
/**
 * Send a mention notification email when a user is @mentioned in a comment.
 */
export const sendMentionEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    ...
    const preview =
      args.commentContent.length > 200
        ? args.commentContent.slice(0, 200) + "…"
        : args.commentContent;

    // Highlight @mentions in preview
    const highlightedPreview = preview.replace(
      /@(\w[\w\s]*)/g,
      `<span style="color:${BRAND_ACCENT};font-weight:600;">@$1</span>`
    );
    ...
    const content = `
      ...
      <div style="padding:40px 40px 36px;">
        ...
        <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#f4f4f5;text-align:center;letter-spacing:-0.5px;">You were mentioned!</h1>
        <p style="margin:0 0 28px;font-size:15px;color:#a1a1aa;text-align:center;line-height:1.6;">
          ${greeting}, <strong style="color:#e4e4e7;">${args.mentionerName}</strong> mentioned you in a comment.
        </p>
        ...
        <div style="background:#0f0f13;border:1px solid #27272a;border-radius:12px;padding:20px 24px;margin-bottom:28px;">
          ...
          <!-- COMMENT QUOTE -->
          <div style="border-left:3px solid ${BRAND_ACCENT};padding:12px 16px;background:#18181f;border-radius:0 8px 8px 0;">
            <p style="margin:0 0 6px;font-size:12px;font-weight:600;color:#71717a;">${args.mentionerName}</p>
            <p style="margin:0;font-size:14px;color:#d4d4d8;line-height:1.6;">${highlightedPreview}</p>
          </div>
        </div>

        <!-- CTA BUTTON -->
        <div style="text-align:center;margin-bottom:24px;">
          ${ctaButton(cardUrl, "View Comment")}
        </div>

        <p style="margin:0;font-size:13px;color:#52525b;text-align:center;line-height:1.6;">
          Go to <strong style="color:#71717a;">${args.boardTitle}</strong> → <strong style="color:#71717a;">${args.cardTitle}</strong> to reply.
        </p>
      </div>`;
    ...
    await resend.emails.send({
      from: getFromEmail(),
      to: args.to,
      subject: `${args.mentionerName} mentioned you in "${args.cardTitle}"`,
      html: baseTemplate(
        content,
        `${args.mentionerName} mentioned you in a comment on "${args.cardTitle}"`
      ),
    });
  },
});
